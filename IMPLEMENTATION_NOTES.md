...
The Fleet UI uses its original event-driven button and panel updates. The general map-only visibility gate and its 0.1-second scan of other UI contexts were rolled back after a reported normal-map FPS regression. This presentation-only rollback does not write save data or change gameplay hooks.
