...
Line selection and frequency use Lua's separate `math.random`, not `Game.Rand`, to avoid perturbing gameplay RNG. Routine chatter is at most one per Azul turn with a 30% base chance; Testudons have a 9% routine chance. Important chatter has a higher chance, but total messages are capped at three per turn and each ship may speak once per turn. The feed is ephemeral, keeps four lines for eight seconds, and hides while the dashboard/selector/confirmation panel is open or the normal map is not active. Chatter is emitted only for the active human Azul player; AI Azul still receives persistent callsigns and kill totals. All new lines are original writing inspired by, not copied from, the original game's radio style.

The Fleet UI centralizes visibility for its button, dashboard, target selector, confirmation, and Comms feed. City enter/exit, popup shown/processed (counted for nesting), leader enter/leave, interface mode, and Bulk Hide/Show events update it immediately. A 0.1-second live-context reconciliation covers screens such as the game menu that queue directly through `UIManager` and do not send the standard popup pair. The UI-only gate does not write save data or change gameplay hooks.
