...
The Fleet UI hides or restores its button, dashboard, target selector, confirmation, and Comms feed only on Civ V's city enter/exit, popup shown/processed, leader enter/leave, Bulk Hide/Show, and interface-mode change events. Popup types are counted so nested popups do not restore the HUD prematurely. Targeting and confirmation close when a modal view opens, while the dashboard's open state is retained. `SetHide` is guarded by the control's current state. Game-data dirty events refresh values but do not recalculate screen visibility. No `UIManager` context scan or visibility timer runs on the map. The Fleet Comms frame callback is installed only while messages are active and cleared when they expire; it only updates message age and alpha, not screen state. The game menu has no reliable matching event in this UI path and may still overlap. The change is presentation-only and writes no save data.
