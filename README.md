...
Ships occasionally react to confirmed kills, damage, losses, near-defeated enemies, heavy targets, city assaults, and their special weapons. Testudons speak rarely; the Main Cannon uses a separate `FLEET SYSTEMS` voice. The feed holds up to four messages and fades after eight seconds. The Fleet Systems HUD hides during City View, diplomacy, popup-backed overviews, Civilopedia, and supported alternate interface modes, then returns on the main map. Targeting and confirmation close safely when another screen opens; an open dashboard returns. The visibility logic uses screen events rather than idle polling. Screens that expose none of these events, such as the game menu, may still overlap the HUD. Every line in this mod is newly written in the style of the original comms, not a quotation from it.
